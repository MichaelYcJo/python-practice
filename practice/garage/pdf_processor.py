...
class PageResult:
    ...
    def __post_init__(self):
        ...
        if self.words:
            self.avg_confidence = sum(w.confidence for w in self.words) / len(
                self.words
            )
...
class DocumentResult:
    ...
    def __post_init__(self):
        ...
        if self.pages:
            ...
            # 평균 신뢰도
            all_confidences = [
                word.confidence for page in self.pages for word in page.words
            ]
            if all_confidences:
            ...
            self.error_pages = [
                p.page_number for p in self.pages if p.status == ProcessingStatus.FAILED
            ]
...
class ImagePreprocessor:
    ...
    def _remove_noise(self, image: Image.Image) -> Image.Image:
        ...
        denoised = cv2.fastNlMeansDenoising(
            img_array, h=10, templateWindowSize=7, searchWindowSize=21
        )
                ...
                if angles:
                    ...
                    if abs(median_angle) > 0.5:  # 0.5도 이상 기울어진 경우만 보정
                        rotated = image.rotate(
                            -median_angle, expand=True, fillcolor="white"
                        )
                        return rotated
    ...
    def _binarize(self, image: Image.Image) -> Image.Image:
        ...
        if CV2_AVAILABLE:
            ...
            # PIL 기반 단순 임계값
            return image.point(
                lambda x: 0 if x < self.config.binarization_threshold else 255, "1"
            )
...
class LanguageDetector:
    ...
    def _ocr_based_detection(self, image: Image.Image, fallback: str) -> Optional[str]:
        ...
        try:
            ...
            return (
                best_lang if best_score > self.config.confidence_threshold else fallback
            )
...
class PerformanceMonitor:
    ...
    def get_summary(self) -> Dict:
        ...
        return {
            ...
            "평균_CPU_사용률": (
                f"{sum(self.metrics['cpu_usage']) / max(len(self.metrics['cpu_usage']), 1):.1f}%"
            ...
            "작업별_시간": {
                name: f"{sum(times):.2f}초" for name, times in self._timers.items()
            },
        }
...
class DPIOptimizer:
    ...
    def _evaluate_dpi(self, pdf_path: Path, page_num: int, dpi: int) -> int:
        ...
        return sum(
            ...
            if data.get("text", [None] * len(data.get("conf", [])))[i]
            and int(conf) >= self.config.confidence_threshold
        )
...
class CheckpointManager:
    ...
    def save_page_result(
        self, checkpoint_path: Path, page_result: PageResult, pdf_bytes: bytes
    ) -> None:
        """페이지 결과 저장"""
        # PDF 저장
        ...
        page_data = {
            ...
            "words": [
                {"text": word.text, "confidence": word.confidence, "bbox": word.bbox}
...
class OCRProcessor:
    ...
    def process_image(
        self, image: Image.Image, page_number: int, language: str = None
    ) -> PageResult:
        """단일 이미지 OCR 처리"""
        ...
        try:
            with self.performance_monitor.time_operation("image_preprocessing"):
                ...
                cache_key = self._generate_cache_key(
                    image, language or self.config.language
                )
            ...
            with self.performance_monitor.time_operation("ocr_extraction"):
                ...
                words = self._extract_words(
                    processed_image, detected_language, page_number
                )
    ...
    def _extract_words(
        self, image: Image.Image, language: str, page_number: int
    ) -> List[WordData]:
        """이미지에서 단어 추출"""
    ...
    def _build_tesseract_config(self) -> str:
        ...
        if self.config.confidence_threshold > 0:
            config_parts.append(
                f"-c tessedit_min_confidence={self.config.confidence_threshold}"
            )
    ...
    def _should_include_word(self, text: str, confidence: int) -> bool:
        ...
        if (
            ...
            or len(text) > self.config.max_word_length
        ):
            return False
    ...
    def _are_words_duplicate(self, word1: WordData, word2: WordData) -> bool:
        ...
        overlap_x = max(
            0,
            min(bbox1["x"] + bbox1["w"], bbox2["x"] + bbox2["w"])
            - max(bbox1["x"], bbox2["x"]),
        )
        overlap_y = max(
            0,
            min(bbox1["y"] + bbox1["h"], bbox2["y"] + bbox2["h"])
            - max(bbox1["y"], bbox2["y"]),
        )
    ...
    def create_searchable_pdf(self, image: Image.Image, language: str) -> bytes:
        ...
        try:
            ...
            return pytesseract.image_to_pdf_or_hocr(
                processed_image, lang=language, extension="pdf", config=config
            )
        except Exception as e:
    ...
    def extract_text_only(self, image: Image.Image, language: str) -> str:
        ...
        try:
            ...
            text = pytesseract.image_to_string(
                processed_image, lang=language, config=config
            )
...
class PDFProcessor:
    ...
    async def process_document(
        ...
        try:
            ...
            result = await self._process_pdf_async(
                file_path, output_dir, checkpoint_path, optimal_dpi, monitor
            )
    ...
    async def _process_pdf_async(
        ...
        # 처리할 페이지 결정
        pending_pages = [
            i for i in range(1, total_pages + 1) if i not in completed_pages
        ]

        if not pending_pages:
            self.logger.info("✅ 모든 페이지가 이미 처리됨")
            return await self._finalize_document(
                pdf_path, checkpoint_path, output_dir, manifest
            )
        ...
        for i in range(0, len(pending_pages), chunk_size):
            ...
            self.logger.info(
                f"📄 청크 처리 중: {i+1}-{min(i+chunk_size, len(pending_pages))} / {len(pending_pages)}"
            )
            ...
            chunk_results = await self._process_pages_parallel(
                pdf_path, chunk, dpi, checkpoint_path, monitor
            )
        ...
        with ProcessPoolExecutor(max_workers=self.config.workers) as executor:
            ...
            for page_num in page_numbers:
                task = loop.run_in_executor(
                    executor, self._process_single_page, pdf_path, page_num, dpi
                )
            ...
            # 완료된 작업 수집
            for completed_task, page_num in asyncio.as_completed(
                [(task, num) for task, num in tasks]
            ):
                try:
                        ...
                        with monitor.time_operation("checkpoint_save"):
                            ...
                            if images:
                                pdf_bytes = self.ocr_processor.create_searchable_pdf(
                                    images[0], result.language
                                )
                                self.checkpoint_manager.save_page_result(
                                    checkpoint_path, result, pdf_bytes
                                )
    ...
    def _process_single_page(
        self, pdf_path: Path, page_number: int, dpi: int
    ) -> PageResult:
        """단일 페이지 처리 (워커 프로세스용)"""
        try:
            # PDF를 이미지로 변환
            ...
            return self.ocr_processor.process_image(
                images[0], page_number, self.config.language
            )
    ...
    async def _finalize_document(
        self, pdf_path: Path, checkpoint_path: Path, output_dir: Path, manifest: Dict
    ) -> DocumentResult:
        """문서 최종화"""
        ...
        if self.config.save_json:
            await self._merge_json_files(
                checkpoint_path, output_pdf.with_suffix(".json")
            )
    ...
    async def _merge_pdfs(self, checkpoint_path: Path, output_path: Path) -> None:
        ...
        await loop.run_in_executor(
            None, lambda: output_path.write_bytes(self._write_pdf_to_bytes(writer))
        )

    async def _merge_json_files(self, checkpoint_path: Path, output_path: Path) -> None:
        """JSON 파일들 병합"""
        merged_data = {"type": "pdf", "pages": []}
...
class ProcessorFactory:
    ...
    def create_processor(
        config: ProcessingConfig, cache: Optional[CacheProtocol] = None
    ) -> PDFProcessor:
        """프로세서 생성"""
...
async def process_documents_batch(
    ...
    if config.progress_bar:
        progress = tqdm(total=total_files, desc="📁 배치 처리", unit="파일", ncols=100)
    else:
        progress = None
...
def create_processing_report(
    results: List[DocumentResult], output_path: Optional[Path] = None
) -> Dict:
    """처리 결과 보고서 생성"""
...
async def example_usage():
    ...
    if file_paths:
        results = await process_documents_batch(
            file_paths, Path("./output"), config, max_concurrent=2
        )

        # 4. 보고서 생성
        report = create_processing_report(
            results, Path("./output/processing_report.json")
        )
